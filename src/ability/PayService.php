...
class PayService
{
     ...
     * var_dump(floor(\Rain\$this->calc(\Rain\$this->calc(0.1, 0.7, '+'), 10, '*'))); // 8
     *
     * @param string $a
     * @param string $b
     ...
     * @throws WechatException
     */
    public function calc($a, $b, $operator, $scale = 2)
    {
        $scale = (int)$scale;
        ...
        if (!array_key_exists($operator, $bc)) {
            throw new WechatException('operator error');
        }
    ...
    public function unifiedOrder($unified, $payKey)
    {
...
//        );
        ...
        $unified['sign'] = $this->getSign($unified, $payKey);
     ...
     * @throws WechatException
     */
    public function notify($checkSign = true)
    {
        ...
        if ($this->getSign($signArr, $config['key']) === $postArr['sign']) {
            ...
            //金额单位转为元
            $postArr['total_fee'] = $this->calc($postArr['total_fee'], 100, '/', 2);
     ...
     * )
     */
    public function refund($out_trade_no, $total_fee, $refund_fee, $out_refund_no, $transaction_id = '')
    {
        $config = $this->getConfig();
...
XML;
     ...
     * )
     */
    public function refundQuery($out_refund_no, $refund_id = '', $out_trade_no = '', $transaction_id = '')
    {
        $config = $this->getConfig();
        $helper = Kernel::getRedpackHelper();
...
XML;
        ...
        if ($responseXml === false) {
            throw new WechatException("curl失败");
        }
